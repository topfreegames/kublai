...
KhanService.prototype.leaveClan = function (gameId, clanId, ownerPublicId, callback) {
  if (!gameId) {
    callback('No game id was provided.', null)
    return
  }
  ...
  if (!ownerPublicId) {
    callback('No owner id was provided.', null)
    return
  }

  const payload = { ownerPublicID: ownerPublicId }
  this.sendJsonRequest('POST', `/games/${gameId}/clans/${clanId}/leave`, payload, (error, res) => {
    callback(error, res)
  })
}

KhanService.prototype.transferClanOwnership = function (gameId, clanId, opts, callback) {
  if (!gameId) {
    callback('No game id was provided.', null)
    return
  }
...
KhanService.prototype.deleteMembership = function (gameId, clanId, opts, callback) {
  if (!gameId) {
    callback('No game id was provided.', null)
    return
  }
