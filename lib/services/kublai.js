...
const KhanService = function (khanUrl, timeout, maxSockets, logger, app, component) {
...
KhanService.prototype.sendJsonRequest = function (operation, method, uri, payload, qs, callback) {
  ...
  let l
  if (this.logger) {
    l = this.logger.child({ operation, service: 'khan' })
  }
  ...
  request({
    ...
    if (l) {
      l.info({ requestDuration: duration, errored: !!error }, 'serviceRequest')
    }
    if (!error) {
...
KhanService.prototype.getClanWithShortId = function (gameId, clanId, callback) {
  if (!gameId) {
    const err = new errors.KhanError('No game id was provided. Operation: getClan.')
    callback(err, null)
    return
  ...
  this.sendJsonRequest('getClan', 'GET', `/games/${gameId}/clans/${clanId}?shortID=true`, callback)
}
